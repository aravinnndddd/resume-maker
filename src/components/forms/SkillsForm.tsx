...
export const SkillsForm: React.FC<SkillsFormProps> = ({ skills, updateSkills }) => {
  ...
  const renderStars = (level: number, skillId?: string, interactive = false) => {
    return (
      <div className="flex items-center space-x-1">
        {[1, 2, 3, 4, 5].map((star) => (
          <Star
            ...
            onClick={interactive && skillId ? () => updateSkillLevel(skillId, star) : undefined}
          />
        ))}
      </div>
    );
  ...
  return (
    <div className="mb-8">
      ...
      <div className="space-y-3">
        {skills.map((skill) => (
          <div key={skill.id} className="flex items-center justify-between p-3 bg-white border border-gray-200 rounded-lg hover:shadow-sm transition-shadow">
